...
class Sigmoid(Function):
    ...
    def backward(ctx: Context, grad_output: Tensor) -> Tensor:
        ...
        # raise NotImplementedError('Need to implement for Task 2.4')
        (x,) = ctx.saved_values
        # der_sig = grad_output.f.mul_zip(x, grad_output.f.add_zip(tensor([1.0]), grad_output.f.neg_map(x)))
        der_sig = grad_output.f.mul_zip(x, 1 + grad_output.f.neg_map(x))
        return grad_output.f.mul_zip(grad_output, der_sig)
...
class Exp(Function):
    ...
    def backward(ctx: Context, grad_output: Tensor) -> Tensor:
        ...
        # raise NotImplementedError('Need to implement for Task 2.4')
        (x,) = ctx.saved_values
        return grad_output.f.mul_zip(grad_output, x)
